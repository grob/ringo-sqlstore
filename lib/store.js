...
const EntityMapping = require("./mapping/entity");
 ...
 * @ignore
 */
const determineDialect = function(connectionPool) {
    const driver = connectionPool.getDriverClassName();
 ...
 * @constructor
 */
const Store = module.exports = function(connectionPool) {
...
Store.prototype.close = function() {
    ...
    this.entityCache && this.entityCache.invalidateAll();
    this.queryCache && this.queryCache.invalidateAll();
};
...
Store.prototype.defineEntity = function(type, definition) {
    const entityMapping = EntityMapping.create(this.dialect, type, definition);
...
Store.prototype.syncTables = function() {
    ...
    try {
        this.entityRegistry.getConstructors().forEach(function(ctor) {
            ...
            if (ctor.mapping.indexes !== null) {
                Object.keys(ctor.mapping.indexes).forEach(function(indexName) {
                    const properties = ctor.mapping.indexes[indexName];
...
Store.prototype.getEntity = function(storable) {
    ...
    const cacheKey = storable._cacheKey;
    const transaction = this.getTransaction();
    const useCache = this.entityCache &&
            (!transaction || !transaction.containsKey(cacheKey));
    if (useCache) {
        entity = this.entityCache.getIfPresent(cacheKey) || constants.LOAD_LAZY;
    }
...
Store.prototype.updateEntity = function(storable, transaction, visited, delayed) {
    ...
    const newEntity = {};
    const isTransient = storable._state === constants.STATE_TRANSIENT;
    if (storable._entity === constants.LOAD_LAZY) {
...
Store.prototype.save = function(storable, transaction) {
    ...
    const delayed = [];
    const visited = new java.util.HashSet();
    try {
...
Store.prototype.get = function(type, id, aggressive) {
    const key = new Key(type, id);
    let entity = null;
    const transaction = this.getTransaction();
    const useCache = this.entityCache &&
            (!transaction || !transaction.containsKey(key.cacheKey));
    if (useCache) {
        entity = this.entityCache.getIfPresent(key.cacheKey);
        if (entity !== null) {
