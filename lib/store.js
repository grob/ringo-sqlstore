...
Store.prototype.syncTables = function() {
    ...
    try {
        for each (let ctor in this.entityRegistry.getConstructors()) {
            ...
            if (!metaData.tableExists(conn, this.dialect, ctor.mapping.tableName)) {
                this.createTable(conn, this.dialect, ctor.mapping);
            }
            if (this.dialect.hasSequenceSupport) {
                let sequence = ctor.mapping.id.sequence;
                if (!metaData.sequenceExists(conn, this.dialect, sequence.name)) {
                    sequence.create(conn, this.dialect);
                }
            }
        }
    } finally {
...
Store.prototype.createTable = function(conn, dialect, mapping) {
    var columns = [];
    for each (let propMapping in mapping.mappings) {
        columns.push(propMapping);
    }
    return dbSchema.createTable(conn, dialect, mapping.schemaName,
            mapping.tableName, columns, [mapping.id.column]);
};
