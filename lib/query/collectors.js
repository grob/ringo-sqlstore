 ...
 * @constructor
 */
const EntityCollector = exports.EntityCollector = function EntityCollector(mapping, alias) {
 ...
 * @param {Transaction} transaction The current transaction, or null
 * @param {Number} offset The offset position of this collection within the resultset
 * @returns The entity
 */
EntityCollector.prototype.collect = function(resultSet, store, transaction, offset) {
    const id = this.mapping.id.dataType.get(resultSet, offset + 1);
    const key = new Key(this.mapping.type, id);
    const cacheKey = key.cacheKey;
    const useCache = store.entityCache !== null &&
            (!transaction || !transaction.containsKey(cacheKey));
    let entity = constants.LOAD_LAZY;
    if (useCache === true) {
 ...
 * @constructor
 */
const AggressiveEntityCollector = exports.AggressiveEntityCollector =
    ...
    Object.defineProperties(this, {
         ...
         * @type Number
         */
        "columnCnt": {"value": mapping.mappings.length, "enumerable": true}
    });
    return this;
};
...
AggressiveEntityCollector.prototype.collect = function(resultSet, store, transaction, offset) {
    const Entity = store.entityRegistry.getConstructor(this.mapping.type);
    const idMapping = this.mapping.id;
    const id = idMapping.dataType.get(resultSet, offset + 1);
    const key = new Key(this.mapping.type, id);
    const cacheKey = key.cacheKey;
    const useCache = store.entityCache !== null &&
            (!transaction || !transaction.containsKey(cacheKey));
    if (useCache === true) {
        const cachedEntity = store.entityCache.get(cacheKey);
        // the stored entity could be a lazy loading one
        if (cachedEntity !== null && cachedEntity !== constants.LOAD_LAZY) {
            return Entity.createInstance(key, cachedEntity);
        }
    }
    const entity = {};
    entity[idMapping.column] = id;
    for (let idx = 1; idx < this.columnCnt; idx += 1) {
        let {column, dataType} = this.mapping.mappings[idx];
        entity[column] = dataType.get(resultSet, offset + 1 + idx);
    }
    if (useCache === true) {
        store.entityCache.put(cacheKey, entity);
    }
    return Entity.createInstance(key, entity);
};
 ...
 * @constructor
 */
const PropertyCollector = exports.PropertyCollector = function PropertyCollector(mapping, alias) {
    Object.defineProperties(this, {
        /**
         * The mapping of this collector
         * @type Mapping
 ...
 * @constructor
 */
const ValueCollector = exports.ValueCollector = function ValueCollector(alias) {
    Object.defineProperties(this, {
        /**
         * Contains the alias for this entity (if specified in the query)
         * @type String
...
ValueCollector.prototype.collect = function(resultSet, store, transaction, offset) {
    ...
    const dataType = jdbcTypes[metaData.getColumnType(offset + 1)];
