...
EntityCollector.prototype.collect = function(resultSet, store, transaction, offset) {
    ...
    if (useCache === true) {
        entity = store.entityCache.getIfPresent(cacheKey);
        if (entity === null) {
...
AggressiveEntityCollector.prototype.collect = function(resultSet, store, transaction, offset) {
    ...
    const id = entity[idMapping.column] = idMapping.dataType.get(resultSet, offset + 1);
    const key = new Key(this.mapping.type, id);
    const cacheKey = key.cacheKey;
    const useCache = store.entityCache !== null &&
            (!transaction || !transaction.containsKey(cacheKey));
    ...
    if (useCache === true) {
        const cachedEntity = store.entityCache.getIfPresent(cacheKey);
