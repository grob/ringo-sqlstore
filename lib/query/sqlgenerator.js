 ...
 * @constructor
 */
const SqlGenerator = module.exports = function(store) {
...
SqlGenerator.prototype.resolveEntityMapping = function(name) {
    const type = this.aliasToEntity[name] || name;
    let mapping = null;
...
SqlGenerator.prototype.visitIdent = function(node) {
    ...
    let alias = this.entityToAlias[propMapping.parent.type] || null;
...
SqlGenerator.prototype.visitSubSelect = function(node) {
    const subGen = new this.store.dialect.SqlGenerator(this.store);
...
SqlGenerator.prototype.visitSelectEntity = function(node) {
    const mapping = this.getEntityMapping(node.name);
    let alias = this.entityToAlias[mapping.type] || null;
