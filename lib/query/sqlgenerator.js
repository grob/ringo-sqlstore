...
const {AggressiveEntityCollector, EntityCollector,
 ...
 * @param {SqlGenerator} parent Optional parent SqlGenerator instance, used
 * to resolve entity aliases in subselects defined in parent statements
 * within them
 * @returns A newly created SqlGenerator instance
 * @constructor
 */
const SqlGenerator = module.exports = function(store, parent) {
...
/**
 * Returns the entity registered under the given alias
 * @param {String} alias The alias
 * @returns {Entity} The aliased entity, or null
 */
SqlGenerator.prototype.getAliasedEntity = function(alias) {
    if (this.aliasToEntity.hasOwnProperty(alias)) {
        return this.aliasToEntity[alias];
    }
    return (this.parent && this.parent.getAliasedEntity(alias)) || null;
};

/**
 * Returns the alias registered for the given entity type
 * @param {String} type The entity type
 * @returns {String} The alias, or null
 */
SqlGenerator.prototype.getEntityAlias = function(type) {
    if (this.entityToAlias.hasOwnProperty(type)) {
        return this.entityToAlias[type];
    }
    return (this.parent && this.parent.getEntityAlias(type)) || null;
};
...
SqlGenerator.prototype.getEntityMapping = function(name) {
    ...
    return this.store.getEntityMapping(this.getAliasedEntity(name) || name);
};
...
SqlGenerator.prototype.resolveEntityMapping = function(name) {
    const type = this.getAliasedEntity(name) || name;
    let mapping = null;
    if (this.store.entityRegistry.hasConstructor(type)) {
        // name refers to an (aliased) entity
        mapping = this.store.getEntityMapping(type);
    } else {
        ...
        mapping = this.entityMappings.find(function(mapping) {
            return mapping.hasSelectableProperty(name)
        });
    }
...
SqlGenerator.prototype.resolvePropertyMapping = function(name, property) {
    ...
    if (this.store.entityRegistry.hasConstructor(type)) {
        // name refers to an (aliased) entity
        mapping = this.store.getEntityMapping(type).getMapping(property || "id");
    } else {
        // see if any of the collected entities of this generator has
        // a property with the given name - if true return the entity mapping
        const entityMapping = this.entityMappings.find(function(mapping) {
            return mapping.hasSelectableProperty(name)
        });
        if (entityMapping) {
            mapping = entityMapping.getMapping(name);
        }
    }
    if (mapping === null || mapping === undefined) {
        if (!this.parent) {
        ...
        mapping = this.parent.resolvePropertyMapping(name, property);
    }
    return mapping;
};
...
SqlGenerator.prototype.visitValue = function(node) {
    if (!dataTypes.hasOwnProperty(node.type)) {
        throw new Error("Unknown data type " + node.type);
    }
    this.params.push({
        "dataType": dataTypes[node.type],
        "value": node.value
    });
    return "?";
};
...
SqlGenerator.prototype.visitOperand = function(node) {
    return "CONCAT(" + node.summands.map(function(summand) {
...
SqlGenerator.prototype.visitEntity = function(node) {
    const mapping = this.getEntityMapping(node.name);
    let result = this.store.dialect.quote(mapping.tableName, mapping.schemaName);
    if (node.alias) {
        result += " " + node.alias;
    }
    return result;
...
SqlGenerator.prototype.visitIdent = function(node) {
    ...
    const propMapping = this.resolvePropertyMapping(node.entity, node.property);
    let alias = this.getEntityAlias(propMapping.parent.type) || null;
    // FIXME: this never seems to happen:
    if (!alias && this.aliasToEntity.hasOwnProperty(node.entity)) {
        alias = node.entity;
    }
    if (alias) {
        return alias + "." + this.store.dialect.quote(propMapping.column);
    }
    return this.store.dialect.quote(propMapping.column, propMapping.parent.tableName);
};
...
SqlGenerator.prototype.visitCondition = function(node) {
    const buf = [node.left.accept(this)];
    if (node.right != null) {
...
SqlGenerator.prototype.visitInCondition = function(node) {
    const buf = [(node.isNot === true) ? "NOT IN (" : "IN ("];
...
SqlGenerator.prototype.visitConditionList = function(node) {
    const str = node.conditions.map(function(condition) {
...
SqlGenerator.prototype.visitSubSelect = function(node) {
    const subGen = new this.store.dialect.SqlGenerator(this.store, this);
    const str = "(" + node.select.accept(subGen) + ")";
...
SqlGenerator.prototype.visitOrderBy = function(node) {
    const buf = [node.value.accept(this)];
    if (node.isReverse) {
        buf.push("DESC");
    } else {
        buf.push("ASC");
...
SqlGenerator.prototype.visitSelectEntity = function(node) {
    const mapping = this.getEntityMapping(node.name);
    let alias = this.getEntityAlias(mapping.type) || null;
    if (!alias && this.getAliasedEntity(node.name)) {
        alias = node.name;
    }
    const buf = [];
    for each (let propMapping in mapping.columns) {
        if (alias) {
            buf.push(alias + "." + this.store.dialect.quote(propMapping.column));
        } else {
            buf.push(this.store.dialect.quote(propMapping.column, propMapping.parent.tableName));
        }
    }
    return buf.join(", ");
};
...
SqlGenerator.prototype.visitSelect = function(node) {
    ...
    const fromClause = node.from.accept(this);
    let joinClause = null;
    if (node.join != null) {
    ...
    if (node.range != null) {
        const limit = !isNaN(node.range.limit) ?
                node.range.limit : node.range.limit.accept(this);
        const offset = !isNaN(node.range.offset) ?
