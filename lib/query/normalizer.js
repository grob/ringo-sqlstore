 ...
 * a query string
 */
const Normalizer = module.exports = function() {
    return this;
};
...
/**
 * Normalizes the operand node
 * @param {Operand} node The operand node
 * @returns {String} The operand node as string
 */
Normalizer.prototype.visitOperand = function(node) {
...
Normalizer.prototype.visitIdent = function(node) {
    const buf = [node.entity];
    if (node.property) {
        buf.push(".", node.property);
    }
    return buf.join("");
};
...
/**
 * Normalizes a nested select statement
 * @param {SubSelect} node The node
 * @returns {String} The string representation
 */
Normalizer.prototype.visitSubSelect = function(node) {
    const str = "(" + node.select.accept(this) + ")";
    if (node.range != null) {
...
Normalizer.prototype.visitExpression = function(node) {
    const buf = [node.andConditions.accept(this)];
...
Normalizer.prototype.visitSelect = function(node) {
    ...
    ["from", "join", "where", "groupBy", "having", "orderBy", "range"].forEach(function(prop) {
        if (node[prop] != null) {
