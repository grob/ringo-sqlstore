 ...
 * @param {Array} values Optional array of values used for replacing placeholders
 * in the filter string (eg. "$1")
 * @returns The parsed filter string
 * @type String
 */
function parseFilterStr(query, str, values) {
    var buf = [];
    str.split(/\s+/).forEach(function(token) {
        if (token.indexOf(".") > -1) {
            var [entityName, property] = token.split(".");
            var entity = query.store.getEntityConstructor(entityName);
            var propMapping = entity.mapping.getMapping(property);
            buf.push(propMapping.getQualifiedColumnName(query.store.dialect));
        } else if (query.mapping.properties[token] != null) {
            var propMapping = query.mapping.getMapping(token);
            buf.push(propMapping.getQualifiedColumnName(query.store.dialect));
        } else if (token.indexOf("$") === 0) {
            // replace with argument value
            buf.push(convertValue(values.shift()));
        } else {
            buf.push(token);
        }
    });
 ...
 * @type String
 */
var getColumnName = function(query, property) {
    if (property.indexOf(".") > -1) {
        var [entityType, propName] = property.split(".");
        mapping = query.store.getEntityConstructor(entityType).mapping;
        property = propName;
    } else {
        mapping = query.mapping;
    }
    var propMapping = mapping.getMapping(property);
    return propMapping.getQualifiedColumnName(query.store.dialect);
};
...
var OperatorClause = function(query, operator, property, value) {
    ...
    this.toSql = function() {
        var columnName = getColumnName(query, property);
...
var OrderClause = function(query, property, order) {
    ...
    this.toSql = function() {
        return getColumnName(query, property) + " " + order;
    };
...
var InnerJoin = function(query, relationEntity, predicate) {
    ...
    this.toSql = function() {
        ...
        sqlBuf.append(" ON ");
        sqlBuf.append(parseFilterStr(query, predicate));
        return sqlBuf.toString();
    };
 ...
 * @private
 */
var FilterClause = function(query, filter, values) {
    var sql = parseFilterStr(query, filter, values);
