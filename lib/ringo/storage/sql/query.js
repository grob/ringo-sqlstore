...
Query.prototype.toSql = function(property) {
    ...
    if (typeof(property) === "string") {
        if (property === "*") {
            ...
            columnName = store.dialect.quote(mapping[property].column);
        }
    } else {
        columnName = store.dialect.quote(mapping.id.column);
    }
...
Query.prototype.select = function(property) {
    ...
    if (typeof(property) === "string" && property !== "*") {
        collector = new collectors.PropertyCollector(mapping, property);
    } else {
        collector = new collectors.EntityCollector(mapping, type);
    }
    return store.executeQuery(sql, mapping, collector);
};
...
Query.prototype.equals = function(property, value) {
    if (value instanceof Array) {
        this.clauses.push(new InClause(property, value));
    } else {
 ...
 * @private
 */
var InClause = function(property, value) {
