...
Query.prototype.select = function(property) {
    ...
    log.debug("Retrieving:", sql);
    var collector = this.store.getCollector(this.mapping, property);
    return this.store.executeQuery(sql, collector);
};
...
Query.prototype.aggregate = function(aggregation, property) {
    var sql = this.toSql(property, aggregation);
    log.debug("Retrieving aggregation:", sql);
    var collector = this.store.getCollector(this.mapping, property);
    return this.store.executeQuery(sql, collector)[0];
};
