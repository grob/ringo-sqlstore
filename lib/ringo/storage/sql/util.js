...
export("close", "createTable", "getColumns", "getPrimaryKeys", "getTables", "tableExists", "dropTable");
...
function createTable(conn, dialect, tableName, columns, primaryKey) {
    ...
    sqlBuf.append("CREATE TABLE ").append(tableName);
    sqlBuf.append(" (");
 ...
 * @param {String} schemaName Optional schema name
 */
function dropTable(conn, tableName, schemaName) {
    var sqlBuf = new java.lang.StringBuffer("DROP TABLE ");
    if (schemaName != null) {
        sqlBuf.append(schemaName).append(".");
    }
    sqlBuf.append(tableName);
    log.info("Dropping", tableName, sqlBuf.toString());
    var statement = null;
    try {
        conn.setReadOnly(false);
        statement = conn.createStatement();
