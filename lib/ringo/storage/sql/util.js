...
function createTable(conn, dialect, schemaName, tableName, columns, primaryKey) {
    ...
    columns.forEach(function(column, idx) {
        ...
        if (!dataType) {
            throw new Error("Unable to determine data type " + column.type);
        }
 ...
 * @type Array
 */
function getTables(conn, schemaName) {
    var result = [];
    var metaData = null;
    var tableMetaData = null;
    try {
        metaData = conn.getMetaData();
        tableMetaData = metaData.getTables(null, schemaName || "%", "%", ["TABLE"]);
...
function tableExists(conn, tableName, schemaName) {
    return getTables(conn, schemaName).map(function(table) {
        return table.name;
    }).indexOf(tableName) > -1;
};
...
function drop(conn, dialect, typeName, objectName, schemaName) {
    ...
    log.info("Dropping:", sqlBuf.toString());
    var statement = null;
    try {
        conn.setReadOnly(false);
        statement = conn.createStatement();
