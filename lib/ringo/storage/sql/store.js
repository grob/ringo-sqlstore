 ...
 * @type Array
 */
Store.prototype.executeQuery = function(sql, mapping, collector) {
    var conn = null;
    var statement = null;
    var resultSet = null;
    try {
        ...
        return collector.collect(this, resultSet, mapping);
    } finally {
...
Store.prototype.getProperties = function(store, entity) {
    ...
    for each (var propMapping in mapping.properties) {
        ...
        if (propMapping.isCollectionMapping()) {
            ...
            if (propMapping.foreignProperty !== null) {
            ...
            if (propMapping.isPartitioned === true) {
                value = new collections.PartitionedCollection(propMapping, query);
            } else {
                value = new collections.Collection(propMapping, query);
            }
...
Store.prototype.loadEntity = function(type, id) {
    ...
    var collector = new collectors.EntityDataCollector(mapping, type);
    var entities = this.executeQuery(sqlBuf.toString(), mapping, collector);
...
Store.prototype.isEntityExisting = function(type, id) {
    ...
    // TODO: use preparedStatement
    var collector = new collectors.PropertyCollector(mapping, mapping.id.name);
    var result = this.executeQuery(sqlBuf.toString(), mapping, collector);
    if (result.length > 1) {
