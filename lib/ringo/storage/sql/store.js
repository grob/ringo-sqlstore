    ...
    this.defineEntity = function(type, mapping) {
        ...
        if (!ctor) {
            ...
            if (sqlUtils.tableExists(conn, mapping.table) === false) {
                ...
                // create ID sequence too, if defined in mapping
                var sequenceName = this.getIdSequenceName(type);
                if (sequenceName != null && this.dialect.hasSequenceSupport()) {
                    sqlUtils.createSequence(conn, this.dialect, sequenceName);
                }
            } else {
                // TODO: update table
            }
...
Store.prototype.generateId = function(type) {
    ...
    if (sequenceName != null && this.dialect.hasSequenceSupport()) {
        ...
        sqlBuf.append(this.dialect.getSqlNextSequenceValue(sequenceName));
    } else {
...
Store.prototype.remove = function(key, transaction) {
    ...
    sqlBuf.append(this.dialect.quote(tableName)).append(" WHERE ");
...
Store.prototype.insert = function(entity, transaction) {
    ...
    sqlBuf.append(this.dialect.quote(mapping.table)).append(" (");
...
Store.prototype.update = function(entity, transaction) {
    ...
    sqlBuf.append(this.dialect.quote(mapping.table)).append(" SET ");
