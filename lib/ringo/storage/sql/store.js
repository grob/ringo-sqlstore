 ...
 * @returns True if the entity is either new or dirty
 * @type Boolean
 */
Store.prototype.updateEntity = function(properties, entity, transaction) {
    ...
    if (transaction.hasKey(entity._key)) {
        // already touched this entity within the transaction
        return false;
    }
    ...
    var isNew = (entity._key.isPersistent() === false);
    var isDirty = false;
    for each (var propMapping in mapping.properties) {
        // ignore collections
        if (propMapping.isCollectionMapping()) {
