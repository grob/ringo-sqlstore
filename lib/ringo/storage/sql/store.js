...
Store.prototype.updateEntity = function(properties, entity) {
    ...
    for (var name in properties) {
        ...
        if (isStorable(value)) {
            value.save();
            value = value._id;
        // FIXME: implement persisting of collection objects (if possible)
        /*
...
Store.prototype.getProperties = function(store, entity) {
    ...
    for (var name in mapping.properties) {
        ...
        if (propMapping.isCollection()) {
            ...
            var mappedType = propMapping.getMappedType();
            var key = new Key(mappedType, value);
            props[name] = this.create(mappedType, key, null);
        } else {
            props[name] = value;
        }
    }
