        ...
        if (!ctor) {
            ...
            if (sqlUtils.tableExists(conn, m.tableName, schemaName) === false) {
                ...
                if (m.hasIdSequence() && this.dialect.hasSequenceSupport()) {
                    sqlUtils.createSequence(conn, this.dialect, m.schemaName, m.idSequenceName);
                }
            } else {
                // TODO: update table
            }
...
Store.prototype.createTable = function(conn, dialect, mapping) {
    ...
    for (var propName in mapping.properties) {
        ...
        if (property.unique === true) {
            primaryKeys.push(property.column);
        }
    }
 ...
 * @type Array
 */
Store.prototype.executeQuery = function(sql) {
    var conn = null;
    var statement = null;
    var resultSet = null;
    try {
        ...
        while (resultSet.next()) {
            ...
            for (var i=1; i<=columnCount; i+=1) {
                ...
                var dataType = this.dialect.getJdbcType(columnType);
                if (dataType == null) {
 ...
 * @param {Array} columns An array containing objects for each column, holding
 * the name of the property, it's value and the mapped data type
 * @param {Transaction} transaction Optional transaction
 * @returns The result as received from the database
 */
Store.prototype.executeUpdate = function(sql, columns, transaction) {
    var conn = null;
    var statement = null;
    try {
        if (transaction != null) {
        ...
        columns.forEach(function(column, idx) {
            if (column.value === undefined || column.value === null) {
                ...
                column.dataType.set(statement, column.value, idx + 1);
            }
        });
...
Store.prototype.generateId = function(type) {
    ...
    var resultSet = null;
    var conn = this.getConnection();
    log.info("Generating ID:", sqlBuf.toString());
    try {
        statement = conn.createStatement();
...
Store.prototype.updateEntity = function(properties, entity) {
    ...
    for each (var propMapping in mapping.properties) {
        ...
        if (propMapping.isMappedToEntity() && propValue != null) {
        ...
        if (isNew === false && isDirty === false && propValue != entityValue) {
            isDirty = true;
        }
...
Store.prototype.insert = function(entity, transaction) {
    ...
    columns.push({
        "name": mapping.idColumnName,
        "value": nextId,
        "dataType": this.dialect.getType("integer")
    });
    ...
    for each (var propMapping in mapping.properties) {
        ...
        valuesBuf.append("?");
        columns.push({
            "name": propMapping.column,
            "value": entity[propMapping.column],
            "dataType": this.dialect.getType(propMapping.type)
        });
    }
    ...
    var result = this.executeUpdate(sqlBuf.toString(), columns, transaction);
...
Store.prototype.getProperties = function(store, entity) {
    ...
    for each (var propMapping in mapping.properties) {
        ...
        if (propMapping.isCollection()) {
            ...
            var localVal = entity[mapping.idColumnName];
            var mappedEntity = propMapping.getMappedType();
            var query = this.query(mappedEntity).equals(foreignProp, localVal);
            value = new Collection(propMapping.name, query);
            // new Collection(this, props, propMapping);
        } else if (propMapping.isMappedToEntity() && value != null) {
            // load the mapped entity from database
            var type = propMapping.getMappedType();
            if (this.isEntityExisting(type, value) === true) {
                var key = new Key(type, value);
                value = this.create(type, key, null);
            }
        }
...
Store.prototype.loadEntity = function(type, id) {
    ...
    log.info("Loading entity:", sqlBuf.toString());
    // TODO: use preparedStatement
    var entities = this.executeQuery(sqlBuf.toString());
...
Store.prototype.isEntityExisting = function(type, id) {
    ...
    sqlBuf.append(this.dialect.quote(mapping.idColumnName));
    sqlBuf.append(" = ").append(id.toString());
    log.info("Checking entity:", sqlBuf.toString());
    // TODO: use preparedStatement
    var result = this.executeQuery(sqlBuf.toString());
    if (result.length > 1) {
        throw new Error("Multiple rows returned by query");
    }
...
Store.prototype.get = function(type, id, aggressive) {
    // FIXME: implement aggressive loading
