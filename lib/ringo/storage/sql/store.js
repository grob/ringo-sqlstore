        ...
        while (resultSet.next()) {
            ...
            if (store.isCacheEnabled()) {
                if (store.cache.containsKey(cacheKey)) {
                    ...
                    result.push(store.create(mapping.type, entity._key, entity));
                    continue;
                }
            }
            // cache miss
            var entity = createEntity(store, mapping, resultSet, metaData, columnCount);
            result.push(store.create(mapping.type, entity._key, entity));
        }
    } else {
        while (resultSet.next()) {
            var key = new Key(mapping.type, idColumnType.get(resultSet, 1));
            result.push(store.create(mapping.type, key, null));
        }
    }
    return result;
}
...
Store.prototype.insert = function(entity, transaction) {
    ...
    // id column
    var nextId = mapping.id.getNextId(transaction);
    ...
    // store the ID in the entity and it's key
    entity[mapping.id.column] = entity._key.id = nextId;
    if (transaction != null) {
...
Store.prototype.get = function(type, id, aggressive) {
    if (this.isCacheEnabled()) {
        ...
        if (this.cache.containsKey(cacheKey)) {
            var entity = this.cache.get(cacheKey);
