 ...
 * @constructor
 */
var Collection = function(name, query, partitionSize) {
    ...
    var partitions = [];
    partitionSize = partitionSize || 100;
    ...
    var initialize = function() {
        if (!isInitialized) {
            // retrieve the IDs of the objects in this collection
            var conn = null;
            var statement = null;
            var resultSet = null;
            var sql = query.toSql();
            var columnType = mapping.columns[mapping.id.column].getColumnType();
            try {
                ...
                log.debug("Retrieving collection IDs:", sql);
                statement = conn.createStatement();
                resultSet = statement.executeQuery(sql);
                ids = [];
                while (resultSet.next()) {
                    ids.push(columnType.get(resultSet, mapping.id.column, 1));
                }
            } finally {
            ...
            partitions = new Array(Math.ceil(ids.length / partitionSize));
        }
        isInitialized = true;
        return;
    };
    ...
    this.get = function(idx) {
        ...
        if (idx >= 0 && idx < ids.length) {
            var partitionIdx = Math.floor(idx / partitionSize);
            var partition = partitions[partitionIdx];
            if (partition == undefined) {
                // load partition
                var start = partitionIdx * partitionSize;
                var end = Math.min(start + partitionSize, ids.length);
            ...
            var objectIdx = idx - (partitionIdx * partitionSize);
