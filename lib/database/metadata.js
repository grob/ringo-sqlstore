 ...
 * @returns {Array} An array containing the table's primary keys
 */
const getIndexes = exports.getIndexes = function(conn, dialect, tableName, schemaName, isUnique, approximate) {
    const indexes = {};
    let indexInfo = null;
    const sorting = {"A": "asc", "D": "desc"};
    try {
 ...
 * "schema" and "name"
 */
const getTables = exports.getTables = function(conn, dialect, schemaName) {
    const result = [];
    let resultSet = null;
    try {
        resultSet = conn.getMetaData().getTables(null,
                schemaName || dialect.getDefaultSchema(conn),
                "%", ["TABLE"]);
 ...
 */
const getSequences = exports.getSequences = function(conn, dialect) {
...
exports.indexExists = function(conn, dialect, tableName, indexName) {
    const indexes = getIndexes(conn, dialect, tableName);
