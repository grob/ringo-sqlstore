...
Store.prototype.syncTables = function() {
    ...
    try {
        for each (let [type, ctor] in Iterator(this.entityRegistry)) {
...
Store.prototype.executeQuery = function(sql, parameters, collectorFunc) {
    ...
    try {
        ...
        return collectorFunc(statement.executeQuery());
    } finally {
...
Store.prototype.remove = function(storable, transaction) {
    ...
    // load the entity if the storable has an onRemove callback method
    // defined, and isn't loaded already
    if (typeof(storable.onRemove) === "function" && storable._entity === Storable.LOAD_LAZY) {
