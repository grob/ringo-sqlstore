...
Store.prototype.getEntity = function(storable) {
    ...
    return this.loadEntity(storable._key.type, storable._key.id);
};
...
Store.prototype.updateEntity = function(storable, transaction, visited) {
    ...
    var properties = storable._props;
    // load entity from DB if necessary
    var entity = storable._entity || (storable._entity = this.getEntity(storable));
    if (storable._state === Storable.STATE_TRANSIENT) {
        // get next ID and store it both in the key and in the entity
        var id = mapping.id.getNextId(transaction);
        storable._key.id = entity[mapping.id.column] = id;
    }
    for each (var propMapping in mapping.properties) {
        // ignore collections
        if (propMapping.isCollectionMapping()) {
...
Store.prototype.insert = function(storable, transaction) {
    ...
    log.debug("Inserting", storable._key, sqlBuf.toString());
    var result = this.executeUpdate(sqlBuf.toString(), parameters, transaction);
    transaction.addInserted(storable._key);
    return result;
};
...
Store.prototype.update = function(storable, transaction) {
    ...
    // execute update
    log.debug("Updating", storable._key, sqlBuf.toString());
    var result = this.executeUpdate(sqlBuf.toString(), parameters, transaction);
    transaction.addUpdated(storable._key);
    // remove object from cache if enabled
    if (this.isCacheEnabled()) {
        this.cache.remove(storable._key.getCacheKey());
    }
    return result;
};
 ...
 * @param {Storable} storable The storable to the the properties object for
 * @returns The properties of the storable
 * @type Object
 */
Store.prototype.getProperties = function(storable) {
    var type = storable._key.type;
    var mapping = this.getEntityMapping(type);
    var entity = storable._entity;
    var props = {};
    for each (var propMapping in mapping.properties) {
        var value = entity[propMapping.column];
        if (propMapping.isCollectionMapping()) {
            value = Collection.createInstance(this, propMapping, entity);
        } else if (propMapping.isObjectMapping()) {
            // load the mapped entity from database
            value = (value != null && this.get(propMapping.entity, value)) || null;
        }
 ...
 * @type Query
 */
Store.prototype.query = function(queryStr) {
    return (new Query(this, queryStr)).select();
};
...
Store.prototype.get = function(type, id, aggressive) {
    ...
    if (key !== null) {
        if (this.isCacheEnabled()) {
            this.cache.put(key.getCacheKey(), [key, entity]);
        }
        return this.create(type, key, entity);
    }
    return null;
