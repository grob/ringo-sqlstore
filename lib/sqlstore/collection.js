    ...
    this.get = function(idx) {
        ...
        if (idx >= 0 && idx < ids.length) {
            ...
            if (partition == undefined) {
                ...
                var idsToFetch = ids.slice(start, end).map(function(obj) {
                    return new ast.IntValue(obj.id);
                });
                var entityName = query.ast.from.list[0].entity;
                var selectClause = new ast.SelectClause([new ast.Entity(entityName, true)]);
                ...
                partition = partitions[partitionIdx] = Query.select(query.store, select, null, []);
            }
