...
AggressiveEntityCollector.prototype.collect = function(resultSet, ids, storables) {
    ...
    var hasEntityCache = store.hasEntityCache();
...
var query = function(store, statement, nparams) {
        ...
        "load": {
            "value": function() {
                ...
                if (useCache && store.entityCache.containsKey(cacheKey)) {
                    ...
                    [ids, storables] = query(store, statement,
        ...
        "value": function(idx) {
            if (idx >= 0 && idx < this.length) {
                ...
                if (partition == undefined) {
                    // load partition
                    var statement = this.getPartitionQueryStatement(partitionIdx);
                    var [ids, storables] = query(this.store, statement);
