 ...
 */
var createCollector = function(store, selector) {
    var node = selector.ast.select.list[0].expression;
    if (node.constructor.name === "SelectEntity") {
        return new AggressiveEntityCollector(getEntityMapping(store,
                selector.aliasToEntity, node.name));
    }
    return new EntityCollector(getEntityMapping(store,
            selector.aliasToEntity, node.entity));
};
    ...
    for each (let propMapping in this.mapping.columns) {
        ...
        if (propMapping === this.mapping.id) {
            ...
            if (hasEntityCache === true) {
                if ((!transaction || !transaction.containsKey(key.cacheKey)) &&
                        store.entityCache.containsKey(key.cacheKey)) {
...
var query = function(store, selector, nparams) {
    ...
    var collector = createCollector(store, selector);
    return store.executeQuery(selector.sql, params, function(resultSet) {
        var ids = [];
        var storables = [];
