...
var {SqlGenerator} = require("./query/sqlfunctiongenerator");
...
var select = function(store, statement, nparams) {
    ...
    return store.executeQuery(sql, params, function(resultSet) {
        ...
        while (resultSet.next()) {
            statement.collector.collect(resultSet, ids, storables);
        }
...
Collection.createInstance = function(store, collectionMapping, parent) {
    var statement = getStatement(store, collectionMapping.query, CollectorGenerator);
