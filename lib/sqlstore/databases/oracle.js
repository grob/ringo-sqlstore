...
Dialect.prototype.addSqlLimit = function(sqlBuf, limit) {
    ...
    return sqlBuf.append(") WHERE ROWNUM <= ").append(limit.toString());
};
...
Dialect.prototype.addSqlOffset = function(sqlBuf, offset) {
    sqlBuf.insert(0, "SELECT * FROM (SELECT r.*, ROWNUM rnum FROM (");
    return sqlBuf.append(") r ) where rnum > ").append(offset.toString());
};
...
Dialect.prototype.addSqlRange = function(sqlBuf, offset, limit) {
    sqlBuf.insert(0, "SELECT * FROM (SELECT r.*, ROWNUM rnum FROM (");
    sqlBuf.append(") r WHERE ROWNUM <= ").append((offset + limit).toString());
    return sqlBuf.append(") WHERE rnum > ").append(offset.toString());
};
