...
/**
 * Time in millis to check if a connection is valid (value: 1 minute)
 * @type Number
 */
Connection.TIME_TO_CHECK = 60000;
...
Connection.prototype.isValid = function() {
    var conn = this.getConnection();
    if (conn.isClosed() || conn.getWarnings()) {
        return false;
    }
    return (new Date()).getTime() - this.getLastUse() < Connection.TIME_TO_CHECK
            || this.validate();
};
...
Connection.prototype.validate = function() {
    try {
        ...
        log.debug("Connection", this, "is dead");
        return false;
    }
    log.debug("Connection", this, "is valid");
    return true;
};
