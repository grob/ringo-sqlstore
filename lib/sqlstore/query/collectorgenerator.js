...
var getCollector = function(store, aliases, selectExpression, offset) {
    ...
    if (node.constructor.name === "SelectEntity") {
        ...
        return new AggressiveEntityCollector(mapping, offset, selectExpression.alias);
    } else if (node.constructor.name === "Ident") {
        mapping = store.getEntityMapping(aliases[node.entity] || node.entity);
        if (node.property === null) {
            return new EntityCollector(mapping, offset, selectExpression.alias);
        }
        return new PropertyCollector(mapping.getMapping(node.property),
