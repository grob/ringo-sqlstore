 ...
 * @param {Array} columns The column names and JDBC types
 * @param {Store} store The store to operate on
 * @returns The entity
 */
EntityCollector.prototype.collect = function(resultSet, columns, store) {
    var id = columns[this.columnIdx].type.get(resultSet, this.columnIdx + 1);
 ...
 * @returns The entity
 */
AggressiveEntityCollector.prototype.collect = function(resultSet, columns, store) {
    var idColumn = columns[this.columnIdx];
    var id = idColumn.type.get(resultSet, this.columnIdx + 1);
    var useCache = store.entityCache !== null;
    var cacheKey = null;
    if (useCache === true) {
        cacheKey = Key.createCacheKey(this.mapping.type, id);
        var transaction = store.getTransaction();
    ...
    if (useCache && store.entityCache.containsKey(cacheKey)) {
        ...
        key = new Key(this.mapping.type, id);
        entity = {};
        entity[idColumn.name] = id;
        var columnIdx = this.columnIdx;
        var max = columnIdx + this.columnCnt;
        for (let i=columnIdx + 1; i<max; i+=1) {
            let column = columns[i];
            entity[column.name] = column.type.get(resultSet, i + 1);
        }
        if (useCache) {
            store.entityCache.put(cacheKey, [key, entity]);
        }
 ...
 */
PropertyCollector.prototype.collect = function(resultSet, columns, store) {
    return columns[this.columnIdx].type.get(resultSet, this.columnIdx + 1);
};
 ...
 * @returns The value
 */
ValueCollector.prototype.collect = function(resultSet, columns, store) {
    return columns[this.columnIdx].type.get(resultSet, this.columnIdx + 1);
};
