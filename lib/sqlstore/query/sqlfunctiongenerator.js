...
SqlFunctionGenerator.prototype.visitSelect = function(node) {
    ...
    this.buffer.push("\"" + sqlbuf.join("").replace(/"/g, "\\\"") + "\",");
...
/*
    ...
    console.log("--------");
*/
    var collectorGenerator = new CollectorGenerator(this.store, this.aliases);
    var collector = collectorGenerator.create(node.select);
    return {
        "collector": collector,
        "create": new Function("nparams", "getParameterDescription", this.buffer.join("\n"))
    };
};
