...
selectShort =
    fc:fromClause jc:joinClause? wc:whereClause? gc:groupByClause? hc:havingClause? oc:orderByClause? rc:rangeClause? {
        var selectEntities = fc.list.map(function(fromExpr) {
            return new ast.SelectEntity(fromExpr.alias || fromExpr.entity.entity);
        });
...
selectExpression =
    expression:(selectAggregation / selectIdent / selectEntity) alias:alias? ws {
        if (alias) {
...
selectAggregation =
    type:(MAX / MIN / SUM / COUNT) LPAREN ident:ident RPAREN {
        return new ast.SelectAggregation(ast.SelectAggregation[type], ident);
    }
