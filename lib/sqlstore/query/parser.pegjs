...
summand =
    left:factor oper:(PLUS / MINUS) right:factor {
        return new ast.Summand(left, oper, right);
    }
    / f:factor {
        return f;
    }
...
factor =
    left:term oper:(MULTIPLY / DIVISION / MODULO) right:term {
        return new ast.Factor(left, oper, right);
    }
    / t:term {
        return t;
    }
