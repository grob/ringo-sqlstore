...
SqlGenerator.prototype.visitIdent = function(node) {
    var propMapping = this.getPropertyMapping(node.entity, node.property);
        ...
        for each (let propMapping in mapping.columns) {
            if (!propMapping.isCollectionMapping()) {
                buf.push(propMapping.getQualifiedColumnName(dialect, alias) +
                    " AS " + node.getColumnAlias(propMapping.name));
            }
        }
        return buf.join(", ");
    }
    var propMapping = mapping.getMapping(node.property);
    return propMapping.getQualifiedColumnName(dialect, alias) +
            " AS " + node.getColumnAlias();
};
...
SqlGenerator.prototype.visitSelectAggregation = function(node) {
    return node.aggregation.accept(this) + " AS " + node.getColumnAlias();
};
