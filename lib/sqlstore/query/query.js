            ...
            "get": function() {
                if (statement === null) {
                    if (store.hasQueryCache() && store.queryCache.containsKey(queryStr)) {
                        ...
                        statement = ast.accept(new SqlFunctionGenerator(store, ast.aliases));
...
Query.select = function(store, statement, nparams) {
    // console.log(statement.compile.toSource());
    var [sql, params] = statement.create(nparams, getParameterDescription);
//    var collectorGenerator = new CollectorGenerator(store, statement.aliases);
//    console.log("\t=> SQL", sql);
//    console.log("\t=> PARAMS", params);
//    var collector = collectorGenerator.create(statement.select);
    var result = store.executeQuery(sql, params, function(resultSet, metaData) {
        var result = [];
        if (statement.collector.constructor === Array) {
            while (resultSet.next()) {
                var obj = {};
                statement.collector.forEach(function(collector) {
            ...
            while (resultSet.next()) {
                result.push(statement.collector.collect(resultSet, metaData, store));
            }
        }
        return result;
    });
...
Query.prototype.select = function(nparams) {
    return Query.select(this.store, this.statement, nparams);
};
