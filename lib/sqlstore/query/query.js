...
//    var collector = collectorGenerator.create(generator.select);
    var result = store.executeQuery(sql, params, function(resultSet, metaData) {
        var result = [];
        if (generator.collector.constructor === Array) {
            while (resultSet.next()) {
                var obj = {};
                generator.collector.forEach(function(collector) {
            ...
            while (resultSet.next()) {
                result.push(generator.collector.collect(resultSet, metaData, store));
            }
        }
        return result;
    });
