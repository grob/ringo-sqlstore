    ...
    this.commit = function() {
        ...
        if (store.entityCache) {
            ...
            for each (let [cacheKey, storable] in Iterator(deleted)) {
                ...
                for each (let [name, propMapping] in Iterator(storable.constructor.mapping.properties)) {
                    if (propMapping.isCollectionMapping) {
                        keys.push(storable[name]._cacheKey);
                        store.entityCache.remove(storable[name]._cacheKey);
                    }
                }
        ...
        if (store.listeners.commit != null) {
            store.emit("commit", {
                "inserted": Object.keys(inserted),
