 ...
 * @returns A newly created Transaction instance
 */
const Transaction = module.exports = function(store) {
    let connection = null;
    let inserted = {};
    let updated = {};
    let deleted = {};
        ...
        if (hasEntityCache === true) {
            Object.keys(collections).forEach(function(cacheKey) {
                ...
                if (collection._state === constants.STATE_CLEAN) {
                    ...
                    store.entityCache.invalidate(cacheKey);
                }
            });
        }
        [inserted, updated].forEach(function(map) {
        ...
        Object.keys(deleted).forEach(function(cacheKey) {
            ...
            if (collectionMappings.length > 0) {
                collectionMappings.forEach(function(mapping) {
                    ...
                    collection._state = constants.STATE_UNLOADED;
                    if (hasEntityCache === true) {
                        store.entityCache.invalidate(collection._cacheKey);
                    }
                });
            }
            if (hasEntityCache === true) {
    ...
    this.rollback = function() {
        ...
        Object.keys(inserted).forEach(function(cacheKey) {
            inserted[cacheKey]._state = constants.STATE_TRANSIENT;
        });
        Object.keys(updated).forEach(function(cacheKey) {
            updated[cacheKey]._state = constants.STATE_DIRTY;
        });
        Object.keys(deleted).forEach(function(cacheKey) {
            deleted[cacheKey]._state = constants.STATE_CLEAN;
        });
        Object.keys(collections).forEach(function(cacheKey) {
            collections[cacheKey]._state = constants.STATE_UNLOADED;
        });
        reset();
    };
